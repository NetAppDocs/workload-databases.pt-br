...
Você pode explorar as economias para suas cargas de trabalho de banco de dados no EBS, FSx for Windows File Server e storage no local usando o Dashboard, a guia Inventory ou a guia Explore savings. Em todos os casos, você pode usar a savings calculator para comparar vários componentes de custo da execução de cargas de trabalho do Microsoft SQL Server, como storage, compute, licença SQL, snapshots e clones para suas cargas de trabalho de banco de dados em sistemas de arquivos FSx for ONTAP em comparação com Elastic Block Store (EBS), FSx for Windows File Server e storage no local.

Se o Workload Factory identificar economia de custos no FSx for ONTAP, você pode implantar o Microsoft SQL sobre o FSx for ONTAP diretamente a partir da calculadora de economia no console do Workload Factory. Quando você tiver várias instâncias do Microsoft SQL Server sobre Elastic Block Store, FSx for Windows File Server ou storage no local, recomendaremos uma configuração do FSx for ONTAP com uma única instância do SQL.

.Economias potenciais para todos os workloads de banco de dados
Visite o *Dashboard* de Bancos de Dados no console do Workload Factory para obter uma visão geral da economia potencial ao executar todas as suas cargas de trabalho de banco de dados no FSx for ONTAP. O bloco *Potential savings* mostra suas cargas de trabalho de banco de dados no EBS e FSx for Windows File Server, economia potencial, porcentagem de economia e um gráfico de barras.
...
Você pode utilizar a calculadora de economia para comparar vários componentes de custo da execução de cargas de trabalho do Microsoft SQL Server, como armazenamento, computação, licença SQL, snapshots e clones para suas cargas de trabalho de banco de dados em sistemas de arquivos FSx for ONTAP em comparação com Elastic Block Store (EBS), FSx for Windows File Server e storage no local. Os sistemas de arquivos FSx for ONTAP podem ser a opção mais econômica para suas cargas de trabalho de banco de dados.
...
A calculadora verifica sua implantação do SQL Server para garantir que os recursos e recursos utilizados correspondam à edição do SQL Server. Aqui estão os principais fatores e condições que a calculadora verifica antes de recomendar um downgrade para Standard Edition:
...
Se a instância do SQL Server avaliada não utilizar nenhum dos recursos Enterprise mencionados acima e atender às restrições de recursos, a calculadora recomendará o downgrade da licença para Standard Edition. A calculadora ajuda você a otimizar os custos de licenciamento do SQL Server sem reduzir o desempenho ou a funcionalidade.
...
Personalize os campos da calculadora para obter a estimativa de economia mais precisa.



==== Personalize as economias para hosts do Microsoft SQL Server
...
Para mudar para o FSx para ONTAP, siga as instruções para <<Implante o Microsoft SQL Server no AWS EC2 usando o FSX for ONTAP,Implantar Microsoft SQL Server no AWS EC2 usando FSx para sistemas de arquivos ONTAP>>.



==== Personalize as economias para hosts Oracle no EBS

Personalize os campos da calculadora para obter a estimativa de economia mais precisa para seus hosts Oracle em execução no Amazon Elastic Block Store (EBS).

.Passos
. Inicie sessão utilizando uma das link:https://docs.netapp.com/us-en/workload-setup-admin/console-experiences.html["experiências de console"^].
. Selecione o menu image:workloads-menu-icon.png["O ícone do menu hambúrguer é usado para navegar até cargas de trabalho como armazenamento, EDA, IA, bancos de dados, VMware e administração."] Em seguida, selecione *Bancos de dados*.
. Em Bancos de Dados, selecione *Explorar economias* e depois *Oracle no EBS*.
. Na calculadora de economia, forneça os seguintes detalhes:
+
.. *Região*: Selecione uma região no menu suspenso.
.. *Modelo de implantação*: Selecione um modelo de implantação no menu suspenso.
.. *Edição Oracle*: selecione a edição Oracle no menu suspenso.
.. *Taxa de alteração de dados mensais (%)*: Insira a porcentagem que o clone e os dados instantâneos mudam em média por mês.
.. *Frequência de instantâneos*: Selecione uma frequência de instantâneos no menu suspenso.
.. *Número de cópias clonadas*: Insira o número de cópias clonadas na configuração do EBS.
.. *Custo mensal do Oracle BYOL ($)*: opcionalmente, insira o custo mensal do Oracle BYOL em dólares.
.. Sob as especificações do EC2, forneça o seguinte:
+
*** *Descrição da máquina*: Opcionalmente, insira um nome para descrever a máquina.
*** *Tipo de instância*: Selecione o tipo de instância EC2 no menu suspenso.
...
O Workload Factory insere as características do host detectadas para que você possa explorar oportunidades de economia automaticamente.
...
==== Explore as economias para hosts do Microsoft SQL Server detectados
...
+
Se o Workload Factory detectar EBS hosts, você será redirecionado para a guia Explorar economias. Se ele não detectar EBS hosts, você será redirecionado para a calculadora para <<Explore as poupanças através da personalização,explore as poupanças através da personalização>>.
...
. Na calculadora de economia, você pode fornecer os seguintes detalhes sobre clones e snapshots em seu armazenamento EBS para obter uma estimativa de economia de custos mais precisa.
+
...
+
Se o Workload Factory detectar hosts FSx para Windows, você será redirecionado para a guia Explorar economias. Se não detectar hosts FSx para Windows, você será redirecionado para a calculadora para <<Explore as poupanças através da personalização,explore as poupanças através da personalização>>.
...
.. *Frequência de instantâneo*: Selecione uma frequência de instantâneo no menu.
+
Se o Workload Factory detectar shadow copies do FSx for Windows, ele definirá o valor padrão como *Daily*. Se não detectar shadow copies, ele definirá o valor padrão como *No snapshot frequency*.
...
. Selecione o menu image:workloads-menu-icon.png["O ícone do menu hambúrguer é usado para navegar até cargas de trabalho como armazenamento, EDA, IA, bancos de dados, VMware e administração."] Em seguida, selecione *Bancos de dados*.
. No menu Databases, selecione *Explorar economias*.
. Em Explorar economias, selecione o tipo de mecanismo *Microsoft SQL Server*.
. Em seguida, selecione a guia *SQL Server on-premises*.
. Na guia SQL Server no local, selecione *Script de avaliação* > *Informações do script de avaliação* e revise as informações fornecidas.
. Selecione *Download assessment script*.
+
O script é uma ferramenta de coleta de dados baseada em PowerShell. Ele coleta e carrega dados de configuração e desempenho do SQL Server para o Workload Factory. O consultor de migração avalia os dados e planeja a implantação do FSx para ONTAP no seu ambiente SQL Server.

. Execute o script no host do SQL Server com um usuário que tenha privilégios suficientes.
. Selecione *Carregar resultados do script* na guia SQL Server no local do NetApp Workload Factory.
...
==== Explore as economias para hosts Oracle detectados
...
. Selecione o menu image:workloads-menu-icon.png["O ícone do menu hambúrguer é usado para navegar até cargas de trabalho como armazenamento, EDA, IA, bancos de dados, VMware e administração."] Em seguida, selecione *Bancos de dados*.
. No menu Databases, selecione *Explorar economias*.
. Em Explorar economias, selecione a guia *Oracle no Elastic Block Store (EBS)*.
+
Se o Workload Factory detectar EBS hosts, você será redirecionado para a guia Explorar economias. Se ele não detectar EBS hosts, você será redirecionado para a calculadora para <<Explore as poupanças através da personalização,explore as poupanças através da personalização>>.

. Em *Explorar economias*, selecione um ou mais hosts de banco de dados em execução no EBS e, em seguida, selecione *Explorar economias*.
. Caso necessário, autentique o host do banco de dados com as credenciais Oracle ou adicionando as permissões Oracle que estiverem faltando.
+
Se a página Explorar economias não carregar dados após a autenticação bem-sucedida, selecione a aba *Inventário* para recarregar os dados e, em seguida, selecione a aba *Explorar economias* novamente.
...
.. *Adicionar hosts*: Opcionalmente, selecione até cinco hosts EBS detectados para incluir no cálculo da economia.
+
O Workload Factory consolida vários hosts Oracle em uma única recomendação de configuração do FSx for ONTAP para otimizar a economia de custos, a menos que os hosts EBS selecionados excedam os limites de throughput, capacidade ou IOPS para um único sistema de arquivos FSx for ONTAP.
...
. No menu Databases, selecione *Explorar economias*.
. Em Explorar economias, selecione o tipo de mecanismo *Microsoft SQL Server*.
. Em seguida, selecione a guia *Oracle Server on-premises*.
. Na guia Oracle Server on-premises, selecione *Script de avaliação* > *Informações do script de avaliação* e revise as informações fornecidas.
. Selecione *Download assessment script*.
+
O script é uma ferramenta de coleta de dados baseada em PowerShell. Ele coleta dados de configuração e desempenho do Oracle Server.

. Execute o script no host Oracle com um usuário que tenha privilégios suficientes.
. Selecione *Carregar resultados do script* e, em seguida, carregue o arquivo de saída do script para o console do NetApp Workload Factory. O migration advisor avalia os dados e planeja a implantação do Amazon FSx for NetApp ONTAP para o seu ambiente Oracle Server.
. Na guia Oracle Server no local, selecione o Oracle Server no local e, em seguida, selecione *Explore savings* para executar uma análise de custos em relação ao FSx for ONTAP.
. Na calculadora de economia, selecione a região para o host local.
. Opcionalmente, forneça os detalhes a seguir sobre clones (cópias de sombra) e snapshots em seu ambiente de banco de dados local para obter uma estimativa de economia de custo mais precisa.
+
.. *Frequência de instantâneo*: Selecione uma frequência de instantâneo no menu.
...
. Para obter resultados mais precisos, atualize as informações do Compute e os detalhes de armazenamento e desempenho.
+
O Workload Factory consolida vários bancos de dados Oracle locais em uma única recomendação de configuração do FSx for ONTAP para otimizar a economia de custos, a menos que os bancos de dados locais selecionados excedam os limites de taxa de transferência, capacidade ou IOPS para um único sistema de arquivos FSx for ONTAP.



.Remoção de host no local
Após analisar as opções de economia para um host Oracle local, você tem a opção de remover o registro do host local do Workload Factory. Selecione o menu de ações do host Oracle local e, em seguida, selecione *Excluir*.
