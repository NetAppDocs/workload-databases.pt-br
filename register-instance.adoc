...
Usuários da AWS GovCloud (US) devem usar o AWS Secrets Manager para autenticar as credenciais do FSx for ONTAP. Para todos os outros usuários, você deve autenticar o FSx for ONTAP com as credenciais do sistema de arquivos.
...
.. Insira o ARN do parâmetro AWS SSM com as credenciais do FSx for ONTAP para contas GovCloud ou o nome de usuário e a senha do sistema de arquivos FSx for ONTAP para todas as outras contas e, em seguida, selecione *Avançar*.
+
Para contas GovCloud, o valor ARN do parâmetro SSM da AWS é fornecido no Parameter Store, em Detalhes do parâmetro, no Console de Gerenciamento da AWS, no seguinte formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. O tipo de parâmetro SSM da AWS deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<fsx_filesystemID>`. Saiba mais sobre como recuperar parâmetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].
...
.. Insira o ARN do parâmetro AWS SSM com as credenciais do FSx for ONTAP para contas GovCloud ou o nome de usuário e a senha do sistema de arquivos FSx for ONTAP para todas as outras contas e, em seguida, selecione *Avançar*.
+
O tipo de parâmetro do AWS SSM deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<fsx_filesystemID>`. Insira o valor para o ARN do parâmetro do AWS SSM no seguinte formato:
`{fsx:{"username":"fsxadmin","password":"pass"}}`. Saiba mais sobre como recuperar parâmetros do SSM em link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].
