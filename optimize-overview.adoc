...
* *Monitoramento contínuo*: Registre recursos para ativar o monitoramento contínuo. Workload Factory analisa seu ambiente de banco de dados regularmente, fornece recomendações contínuas para otimizar suas configurações e corrige problemas automaticamente.
...
Para a avaliação única, você só precisa fornecer as credenciais uma vez para as instâncias do Microsoft SQL Server ou o banco de dados Oracle que deseja analisar.

Para monitoramento contínuo, você precisa registrar seus recursos, associar um link e conceder permissões em sua conta da AWS.
...
* *Atualizações críticas de segurança da Oracle*
+
...
+
Habilite a replicação entre regiões (CRR) para aprimorar a disponibilidade de dados e os recursos de recuperação de desastres para seus bancos de dados Oracle. A CRR ajuda a proteger contra interrupções regionais e garante a continuidade dos negócios ao replicar dados em locais geograficamente dispersos.

* *Instantâneos consistentes com a aplicação*
+
Use snapshots consistentes de aplicativos com NetApp SnapCenter para tirar snapshots precisos e confiáveis dos seus dados de volume em um momento específico. Isso mantém seus aplicativos funcionando sem problemas e seus dados seguros. SnapCenter facilita os backups e ajuda você a restaurar dados de forma rápida e correta, reduzindo o tempo de inatividade e protegendo suas cargas de trabalho mais importantes.
