...
summary: Crie um novo servidor SQL PostgreSQL dentro do NetApp Workload Factory for Databases. 
---
= Crie um servidor PostgreSQL no NetApp Workload Factory
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
A criação de um novo servidor PostgreSQL ou host de banco de dados no NetApp Workload Factory for Databases requer uma implantação do sistema de arquivos FSx for ONTAP e recursos para o Active Directory.



== Sobre esta tarefa

Antes de criar um servidor PostgreSQL a partir do Workload Factory, saiba mais sobre os tipos de implantação de armazenamento disponíveis para a configuração do host do banco de dados, os modos de operação do Workload Factory e os requisitos para concluir esta operação.
...
+ O FSx para sistemas de arquivos ONTAP tem dois modelos de implantação de servidor PostgreSQL: _Alta Disponibilidade (HA)_ ou _instância única_.  Diferentes recursos são criados para o sistema de arquivos FSx for ONTAP dependendo do modelo de implantação do FSx for ONTAP selecionado.
...
Você deve ter link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["conceder permissões de _criação de host de banco de dados_"^] Na sua conta da AWS, crie um novo host de banco de dados no Workload Factory.
...
. No bloco Bancos de dados, selecione *Implantar host* e, em seguida, selecione *Servidor PostgreSQL* no menu.
...
+
Para contas GovCloud, insira o valor ARN do parâmetro SSM da AWS para as credenciais no seguinte formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. O tipo de parâmetro SSM da AWS deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<anyname>`. Saiba mais sobre como recuperar parâmetros SSM em link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].
