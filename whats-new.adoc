...
link:https://docs.netapp.com/us-en/workload-databases/register-instance.html["Registre recursos no Workload Factory for Databases"]
...
=== Análise de custos e planejamento de migração de ambientes de banco de dados SQL Server on-premises

BlueXP workload factory for Databases agora detecta, analisa e ajuda você a planejar a migração de banco de dados SQL Server no storage no local para Amazon FSx for NetApp ONTAP. Você pode usar a calculadora de economia para estimar o custo de execução do seu ambiente de banco de dados SQL Server no storage no local na nuvem e consultar as recomendações para migrar seu ambiente de banco de dados no storage no local para a nuvem.
