...
---
= Crie um Microsoft SQL Server no NetApp Workload Factory para bancos de dados
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
Você deve link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["conceder permissões de _criação de host de banco de dados_"^] Na sua conta da AWS, crie um novo host de banco de dados no Workload Factory.
...
+
Para contas GovCloud, insira o valor do ARN do parâmetro AWS SSM no seguinte formato:
`{fsx:{"username":"fsxadmin","password":"pass"}}`.

+
O valor é fornecido no Parameter Store, em Detalhes do parâmetro, no Console de Gerenciamento da AWS. O tipo de parâmetro do AWS SSM deve ser "Secure String" com o nome no formato  `/netapp/wlmdb/<anyname>`. Saiba mais sobre como recuperar parâmetros do SSM em link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].
...
+
Para contas GovCloud, insira o valor ARN do parâmetro AWS SSM para credenciais do Active Directory no seguinte formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. O tipo de parâmetro AWS SSM deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<anyname>`. Saiba mais sobre como recuperar parâmetros SSM em link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].
...
+
Para contas GovCloud, insira o valor ARN do parâmetro AWS SSM para as credenciais do FSx for ONTAP no seguinte formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. O tipo de parâmetro AWS SSM deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<anyname>`. Saiba mais sobre como recuperar parâmetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].

+
Um novo sistema de arquivos FSX for ONTAP pode adicionar 30 minutos ou mais de tempo de instalação.

... *Selecione um FSx for ONTAP existente*: Selecione o nome do FSx for ONTAP e insira um nome de usuário e senha para o sistema de arquivos.
+
...
.. *Grupo de segurança*: Um grupo de segurança de carga de trabalho é criado para permitir as portas e os protocolos necessários para a comunicação do cluster Microsoft SQL e Windows nos nós. Além disso, você pode selecionar um ou mais grupos de segurança.
...
+
Opcional: Selecione *Usar conta de serviço gerenciada* para a conta de serviço do SQL Server. Use esta opção se o seu ambiente usar MSA (Conta de Serviço Gerenciada) ou Contas de Serviço Gerenciadas em Grupo (gMSA), onde o gerenciamento de senhas é feito pelo Active Directory.
...
... Insira o nome de usuário e a senha do domínio do Active Directory.
+
Para contas GovCloud, insira o valor ARN do parâmetro AWS SSM para credenciais do Active Directory no seguinte formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. O tipo de parâmetro AWS SSM deve ser "Secure String" com o nome no formato `/netapp/wlmdb/<anyname>`. Saiba mais sobre como recuperar parâmetros SSM em link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["Documentação da AWS GovCloud"^].

... *Controlador de domínio preferencial*: Opcionalmente, insira o controlador de domínio preferencial a ser usado para o Active Directory ingressar.
... *Caminho da unidade organizacional preferencial*: Opcionalmente, insira a unidade organizacional (UO) preferencial no Active Directory à qual deseja ingressar.
... *Grupo do Active Directory de destino*: Opcionalmente, insira o grupo do Active Directory ao qual deseja adicionar os computadores.
